    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-900">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-slate-400 font-semibold text-sm">Loading Department Analytics...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col h-full bg-slate-900 min-h-screen">
        <div className="bg-slate-950 text-white px-6 py-4 border-b border-slate-800 flex justify-between items-center">
          <h1 className="font-heading text-xl font-bold">HOD Performance Dashboard</h1>
        </div>
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="bg-slate-800/80 border border-slate-700 p-8 rounded-2xl max-w-md w-full text-center space-y-4 shadow-xl">
            ...
            <h3 className="font-heading text-lg font-bold text-white">Unable to Load Analytics</h3>
            <p className="text-xs text-slate-400">{errorMessage}</p>
            <button
              onClick={fetchDashboard}
              className="inline-flex items-center px-4 py-2 bg-teal-500 hover:bg-teal-600 text-slate-950 font-bold text-xs rounded-xl shadow transition cursor-pointer"
            >
    ...
    <div className="flex flex-col min-h-screen bg-slate-900 text-slate-100 pb-20">
      ...
      <div className="bg-slate-950/90 backdrop-blur-md px-6 py-4 sticky top-0 z-30 border-b border-slate-800 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-heading text-xl font-bold text-white tracking-tight">{deptName} Analytics</h1>
          </div>
          <p className="text-xs text-slate-400 mt-0.5 font-medium">HOD Dashboard • Dept Code: {deptCode}</p>
        </div>
      ...
      <div className="p-4 sm:p-6 max-w-7xl mx-auto w-full space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-slate-800/90 border border-slate-700/80 p-4 rounded-2xl flex flex-col justify-between h-[105px] shadow-sm">
            <div className="flex justify-between items-center">
              <span className="text-xs font-semibold text-slate-400">Total Teachers</span>
          ...
          <div className="bg-slate-800/90 border border-slate-700/80 p-4 rounded-2xl flex flex-col justify-between h-[105px] shadow-sm">
            <div className="flex justify-between items-center">
              <span className="text-xs font-semibold text-slate-400">Discipline Score</span>
          ...
          <div className="bg-slate-800/90 border border-slate-700/80 p-4 rounded-2xl flex flex-col justify-between h-[105px] shadow-sm">
            <div className="flex justify-between items-center">
              <span className="text-xs font-semibold text-slate-400">Avg Student XP</span>
              <div className="p-2 bg-amber-500/10 text-amber-400 rounded-xl">
                <Zap className="w-4 h-4" />
              </div>
            </div>
            <div className="flex items-baseline gap-1">
              <span className="text-2xl font-bold text-amber-400">{avgXp}</span>
              <span className="text-xs text-slate-400 font-semibold">XP ({totalSections} Secs)</span>
            </div>
          </div>
        </div>
        ...
        {activeSubTab === 'attendance' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="bg-slate-800/70 border border-slate-700/70 p-4 rounded-2xl">
                <span className="text-xs font-semibold text-slate-400">Full Present</span>
              ...
              <div className="bg-slate-800/70 border border-slate-700/70 p-4 rounded-2xl">
                <span className="text-xs font-semibold text-slate-400">Absences</span>
                <div className="text-2xl font-bold text-rose-400 mt-1">
                  {attendance.fullAbsentCount ?? 0} <span className="text-sm text-slate-400 font-normal">Full</span> / {attendance.partialAbsentCount ?? 0} <span className="text-sm text-slate-400 font-normal">Part</span>
                </div>
            ...
            {attendance.sectionAttendance && attendance.sectionAttendance.length > 0 && (
              <div className="bg-slate-800/70 border border-slate-700/70 p-5 rounded-2xl space-y-3">
                <h3 className="font-heading text-sm font-bold text-white">Section-wise Attendance Rates</h3>
            ...
            <div className="bg-slate-800/80 border border-slate-700 p-5 rounded-2xl space-y-4">
              <div className="flex flex-col md:flex-row justify-between md:items-center gap-3">
                <div>
                  <h3 className="font-heading text-base font-bold text-white">Daily Attendance Matrix</h3>
                  <p className="text-xs text-slate-400">Period-wise student attendance log for selected date</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {lookupYears.length > 0 && (
                    <select
                      ...
                      className="bg-slate-900 text-xs font-bold text-slate-200 border border-slate-700 px-3 py-1.5 rounded-xl outline-none cursor-pointer"
                    >
                      {lookupYears.map((y) => (
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-900 border border-slate-700 rounded-xl pl-9 pr-4 py-2 text-xs font-medium text-white placeholder-slate-500 outline-none focus:border-teal-500 transition"
                />
              </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-slate-800/70 border border-slate-700/70 p-4 rounded-2xl">
                ...
                <div className="text-2xl font-bold text-sky-400 mt-1">{xp.totalXp ?? 0} XP</div>
                <span className="text-[11px] text-slate-500 font-medium">All Department Students</span>
              </div>
              <div className="bg-slate-800/70 border border-slate-700/70 p-4 rounded-2xl">
                <span className="text-xs font-semibold text-slate-400">Awarded XP</span>
                <div className="text-2xl font-bold text-emerald-400 mt-1">+{xp.awardXp ?? 0} XP</div>
                <span className="text-[11px] text-slate-500 font-medium">Positive Rewards</span>
              </div>
              <div className="bg-slate-800/70 border border-slate-700/70 p-4 rounded-2xl">
                <span className="text-xs font-semibold text-slate-400">Penalty Deductions</span>
                <div className="text-2xl font-bold text-rose-400 mt-1">-{xp.penaltyXp ?? 0} XP</div>
                <span className="text-[11px] text-slate-500 font-medium">Discipline Violations</span>
              </div>
              <div className="bg-slate-800/70 border border-slate-700/70 p-4 rounded-2xl">
                <span className="text-xs font-semibold text-slate-400">Net XP Growth</span>
                <div className="text-2xl font-bold text-indigo-400 mt-1">{xp.netXp ?? 0} XP</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-slate-800/80 border border-slate-700 p-5 rounded-2xl space-y-3">
                <div className="flex items-center gap-2 text-emerald-400">
                  ...
                  <h3 className="font-heading text-sm font-bold text-white">Top 5 XP Achievers</h3>
                </div>
                <div className="space-y-2">
                  {!xp.topStudents || xp.topStudents.length === 0 ? (
                    <p className="text-xs text-slate-500 py-4 text-center">No student XP data available</p>
                  ) : (
                    xp.topStudents.map((s: any, idx: number) => (
                      <div key={idx} className="bg-slate-900/60 p-3 rounded-xl border border-slate-700/60 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-xs font-bold text-white">{s.studentName}</p>
                            <p className="text-[10px] text-slate-400 font-mono">{s.regNo} • Sec {s.sectionName || '-'}</p>
                          </div>
                        </div>
                        <span className="text-xs font-bold text-emerald-400">+{s.totalXp ?? s.xp} XP</span>
                      </div>
                    ))
                  )}
                </div>
              ...
              <div className="bg-slate-800/80 border border-slate-700 p-5 rounded-2xl space-y-3">
                <div className="flex items-center gap-2 text-amber-400">
                  ...
                  <h3 className="font-heading text-sm font-bold text-white">Students Needing Focus</h3>
                </div>
                <div className="space-y-2">
                  {!xp.lowestStudents || xp.lowestStudents.length === 0 ? (
                    <p className="text-xs text-slate-500 py-4 text-center">No student records needing focus</p>
                  ) : (
                    xp.lowestStudents.map((s: any, idx: number) => (
                      <div key={idx} className="bg-slate-900/60 p-3 rounded-xl border border-slate-700/60 flex items-center justify-between">
                        ...
                        <span className="text-xs font-bold text-amber-400">{s.totalXp ?? s.xp} XP</span>
                      </div>
                    ))
                  )}
                </div>
        ...
        {activeSubTab === 'discipline' && (
          <div className="space-y-6">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-slate-800/70 border border-slate-700/70 p-4 rounded-2xl">
                <span className="text-xs font-semibold text-slate-400">Discipline Average</span>
                <div className="text-2xl font-bold text-teal-400 mt-1">
                  {(discipline.avgScore ?? avgDisciplineScore).toFixed(1)} / 100
                </div>
                <span className="text-[11px] text-slate-500 font-medium">Department Average</span>
              </div>
              <div className="bg-slate-800/70 border border-slate-700/70 p-4 rounded-2xl">
                <span className="text-xs font-semibold text-slate-400">Clean Record Rate</span>
                <div className="text-2xl font-bold text-emerald-400 mt-1">
                  {(discipline.cleanRecordPct ?? 100).toFixed(1)}%
                </div>
                <span className="text-[11px] text-slate-500 font-medium">Zero Penalties</span>
              </div>
              <div className="bg-slate-800/70 border border-slate-700/70 p-4 rounded-2xl">
                <span className="text-xs font-semibold text-slate-400">Total Penalties</span>
                <div className="text-2xl font-bold text-rose-400 mt-1">
                  {discipline.totalPenalties ?? recentPenalties.length}
                </div>
                <span className="text-[11px] text-slate-500 font-medium">Recorded Deductions</span>
              </div>
              <div className="bg-slate-800/70 border border-slate-700/70 p-4 rounded-2xl">
                <span className="text-xs font-semibold text-slate-400">At-Risk Students</span>
            ...
            <div className="bg-slate-800/80 border border-slate-700 p-5 rounded-2xl space-y-4">
              <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                <div>
                  <h3 className="font-heading text-base font-bold text-white">Recent Penalty History</h3>
                  <p className="text-xs text-slate-400">Disciplinary actions logged for students in this department</p>
                </div>
                <div className="relative w-full sm:w-64">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-900 border border-slate-700 rounded-xl pl-8 pr-3 py-1.5 text-xs text-white placeholder-slate-500 outline-none focus:border-teal-500"
                  />
                </div>
              </div>
                      ...
                      <div key={idx} className="bg-slate-900/60 p-3.5 rounded-xl border border-slate-700/60 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            ...
                            {p.sectionName && (
                              <span className="text-[10px] text-teal-300 bg-teal-950 px-1.5 py-0.5 rounded border border-teal-800">
                                Sec {p.sectionName}
                              </span>
                            )}
                          </div>
                        ...
                        <span className="text-xs font-bold px-2.5 py-1 bg-rose-500/20 text-rose-400 border border-rose-500/30 rounded-lg shrink-0">
                          -{p.penaltyXp ?? 0} XP
                        </span>
                      </div>
                    ))}
        ...
        {activeSubTab === 'leaderboard' && (
          <div className="bg-slate-800/80 border border-slate-700 p-5 rounded-2xl space-y-4">
            <div>
              <h3 className="font-heading text-base font-bold text-white">Department Leaderboard</h3>
              <p className="text-xs text-slate-400">Top ranked students ranked by net XP and discipline score</p>
            </div>
                ...
                {leaderboard.map((s: any, idx: number) => {
                  ...
                  return (
                    <div key={idx} className="bg-slate-900/60 p-3.5 rounded-xl border border-slate-700/60 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span className={`w-8 h-8 rounded-full flex items-center justify-center text-xs ${rankColor}`}>
                          #{rank}
                        </span>
                        <div>
                          <p className="text-xs font-bold text-white">{s.studentName}</p>
                      ...
                      <div className="text-right">
                        <span className="text-xs font-bold text-teal-400">{s.netXp ?? s.totalXp ?? 0} XP</span>
        ...
        {activeSubTab === 'sections' && (
          <div className="space-y-4">
            <h3 className="font-heading text-base font-bold text-white">Section Comparison Overview</h3>
                  ...
                  return (
                    <div key={idx} className="bg-slate-800/80 border border-slate-700 p-5 rounded-2xl space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-xs font-semibold text-slate-400">{sec.totalStudents} Students</span>
                      </div>
