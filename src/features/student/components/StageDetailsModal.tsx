  ...
  return (
    <div className="fixed inset-0 bg-slate-50 z-[90] flex flex-col overflow-y-auto animate-in slide-in-from-right duration-200">
      ...
      <div className="bg-white px-6 py-4 sticky top-0 z-10 flex items-center gap-4 border-b border-slate-100 shadow-sm">
        <button
          onClick={onClose}
          className="p-2 -ml-2 text-slate-900 type-btn hover:bg-slate-100 rounded-full transition"
          title="Back"
        >
          <ArrowLeft className="w-6 h-6" />
        </button>
        <h1 className="type-h3 text-slate-900">{stage.name}</h1>
      </div>
      ...
      {isLocked && (
        <div className="fixed bottom-0 inset-x-0 bg-slate-800 text-white py-3.5 px-6 text-center type-caption font-bold shadow-lg z-20">
